...
exports.default = strapi_1.factories.createCoreService('api::moderation-report.moderation-report', ({ strapi }) => ({
    ...
    async keep(reportId, moderatorId) {
    ...
    async remove(reportId, moderatorId) {
        ...
        try {
            if (report.targetType === 'announcement') {
                await strapi.entityService.update('api::announcement.announcement', report.targetId, {
                    data: { etat: 'supprimée' }
                });
            }
            else if (report.targetType === 'carpool') {
                await strapi.entityService.update('api::carpool.carpool', report.targetId, {
