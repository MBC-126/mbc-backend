...
exports.default = async (policyContext, _config, { strapi }) => {
    try {
        ...
        const authHeader = (req.headers && req.headers['authorization']) || (policyContext.headers && policyContext.headers['authorization']);
        const method = (req.method || 'GET').toUpperCase();
        const url = req.url || req.originalUrl || '';
        try {
            strapi.log.debug(`[require-api-token] ${method} ${url}`);
        }
        catch { }
        if (!authHeader || !authHeader.toLowerCase().startsWith('bearer ')) {
            console.warn('[require-api-token] Missing or invalid Authorization header');
            try {
                strapi.log.warn('[require-api-token] Missing or invalid Authorization header');
            }
            catch { }
        ...
        if (!token) {
            ...
            try {
                strapi.log.warn('[require-api-token] Empty Bearer token');
            }
            catch { }
            return policyContext.unauthorized ? policyContext.unauthorized() : false;
        }
        ...
        try {
            strapi.log.debug(`[require-api-token] hash:${hashed === null || hashed === void 0 ? void 0 : hashed.slice(0, 8)}…`);
        }
        catch { }
        ...
        if (!apiToken) {
            ...
            try {
                strapi.log.warn('[require-api-token] API token not found');
            }
            catch { }
            return policyContext.unauthorized ? policyContext.unauthorized() : false;
        }
        ...
        try {
            strapi.log.debug(`[require-api-token] type:${type} need:${needed} allowed:${allowed}`);
        }
        catch { }
        if (!allowed) {
            console.warn('[require-api-token] Token type not allowed for this method');
            try {
                strapi.log.warn('[require-api-token] Token type not allowed for this method');
            }
            catch { }
        ...
        try {
            strapi.log.error(`[require-api-token] Error: ${(e === null || e === void 0 ? void 0 : e.message) || e}`);
        }
        catch { }
        return policyContext.unauthorized ? policyContext.unauthorized() : false;
    }
