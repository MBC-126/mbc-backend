 ...
 */
function startAllCronJobs(strapi) {
 ...
 */
function startNotificationCleanup(strapi) {
...
async function cleanOldNotifications(strapi) {
    try {
 ...
 */
function startExpiringAnnouncementsCheck(strapi) {
    setInterval(() => {
        const now = new Date();
...
async function checkExpiringAnnouncements(strapi) {
    try {
 ...
 */
function startReservationReminders(strapi) {
    setInterval(() => {
        const now = new Date();
...
async function sendReservationReminders(strapi) {
    var _a;
    try {
        ...
        for (const reservation of tomorrowReservations) {
            if (reservation.user && reservation.user.id) {
                const infraName = ((_a = reservation.infrastructure) === null || _a === void 0 ? void 0 : _a.name) || 'Infrastructure';
 ...
 */
function startOldCarpoolsCleanup(strapi) {
    // Nettoyer immédiatement au démarrage
...
async function cleanOldCarpools(strapi) {
    try {
