...
export default factories.createCoreController('api::infrastructure.infrastructure' as any, ({ strapi }: any) => ({

  /**
   * Ajouter une période d'indisponibilité (blackout)
   * Manager only
   */
  async addBlackout(ctx: any) {
    const user = ctx.state.user;
    console.log('📅 addBlackout appelé par user:', user?.id);
   ...
   * Manager only
   */
  async deleteBlackout(ctx: any) {
    const user = ctx.state.user;
    console.log('🗑️ deleteBlackout appelé par user:', user?.id);
